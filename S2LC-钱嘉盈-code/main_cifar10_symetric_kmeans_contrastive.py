...
class ImageSet_dataset(Dataset):
    def __init__(self, mat_path,transform = None, train = True,noisy_rate = 0.1):
...
#                        Noisy[i] = 1
            ...
            #=======================symmetric noise============
            for i in range(50000):
                if np.random.random() < noisy_rate:
...
idx = 0
for noise_rate in np.arange(0.1,0.6,0.1):
...
#        lr = checkpoint['learning rate']
        ...
        optimizer = optim.SGD(net.parameters(), lr=lr,
                              momentum=0.9, weight_decay=5e-4)
        best_acc = 0
        psuedo_label_acc = np.zeros(130)
        network_acc = np.zeros(130)
        ...
        noise_network_acc,total_acc,clean_kmeans_acc,clean_network_acc,adjust_label,adjust_label2,use_idx \
            ...
            psuedo_label_acc[epoch] = noisy_kmeans_acc
            network_acc[epoch] = noise_network_acc
...
#            print('use sample num',use_idx.shape[0])
            ...
            if epoch == 40:                     #start training with psuedo label
                for p in optimizer.param_groups:
                    p['lr'] = 0.001
        Last_ACC[idx,exp] = test_acc
    idx += 1

for noise in range(5):
    for exp in range(3):
        print('sym noise rate:{},exp:{}:'.format(np.arange(0.1,0.6,0.1)[noise],exp))
        print('best acc:{}'.format(Best_ACC[noise,exp]))
        print('laet acc:{}'.format(Last_ACC[noise,exp]))

np.savetxt('./result/sym_best_acc_kmeans_contrastive.csv',Best_ACC,delimiter=',')
np.savetxt('./result/sym_last_acc_kmeans_contrastive.csv',Last_ACC,delimiter=',')
# np.savetxt('./result/sym_kmeans_clean_contrastive.csv',kmeans_clean,delimiter=',')
# np.savetxt('./result/sym_kmeans_noisy_contrastive.csv',kmeans_noisy,delimiter=',')
# np.savetxt('./result/sym_network_clean_contrastive.csv',network_clean,delimiter=',')
# np.savetxt('./result/sym_network_noisy_contrastive.csv',network_noisy,delimiter=',')
