...
#    print('\nEpoch: %d' % epoch)
    ...
    for batch_idx, (inputs, targets, noisy,real_label,idx) in enumerate(trainloader):
        ...
        #     contrastive_loss = 0*cont_loss_func(fea[c_idx], targets[c_idx])
        #     if epoch < crf_step:
...
#                         % (test_loss/(batch_idx+1), 100.*correct/total, correct, total))
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_fmnist.pth')
        best_acc = acc
    return acc,best_acc
...
idx = 0
for noise_rate in np.arange(0.2,1,0.2):
...
np.savetxt('./result/f_mnist_sym_best_acc.csv',best_acc_exp)
np.savetxt('./result/f_mnist_sym_last_acc.csv',last_acc_exp)
